...
int main( int argc, char **argv )
{
  ...
  if( argc < 3 ) {
    cerr << "usage: abstractor PSVN_file output_prefix" << endl;
    exit( -1 );
  }
...
/* read, check, and record all the abstraction input */
  ...
  while( 1 ) {
    ...
    /* The input for one instruction has now all been read, now apply the abstractions to the given PSVN.
  ...
  } // end of the while loop for reading in instructions to define the abstraction
